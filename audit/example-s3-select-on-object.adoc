...
summary: Cuando un cliente S3 emite una consulta S3 Select en un objeto, se generan mensajes de auditoría y se guardan en el registro de auditoría. 
---
...
Cada consulta genera dos mensajes de auditoría: Uno que realiza la autorización de la solicitud Select de S3 (el campo S3SR se establece en “SELECT”) y una operación GET estándar posterior que recupera los datos del almacenamiento durante el procesamiento.
