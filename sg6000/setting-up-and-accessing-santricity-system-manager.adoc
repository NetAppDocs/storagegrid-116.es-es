...
+
Si quiere más información sobre E-Series AutoSupport, consulte http://["Sitio de documentación para sistemas E-Series y EF-Series de NetApp"^].
...
NOTE: Si el instalador de dispositivos StorageGRID no está disponible o no aparece la página de inicio de sesión, debe utilizar el xref:setting-ip-addresses-for-storage-controllers-using-storagegrid-appliance-installer.adoc[Las direcciones IP para las controladoras de almacenamiento]. Para acceder a SANtricity System Manager, vaya a la IP de la controladora de almacenamiento.
...
+
Para obtener instrucciones específicas sobre la configuración de un proxy StorageGRID para enviar mensajes de AutoSupport E-Series sin utilizar el puerto de gestión, vaya al xref:../admin/configuring-storage-proxy-settings.adoc[instrucciones para configurar la configuración del proxy de almacenamiento].
