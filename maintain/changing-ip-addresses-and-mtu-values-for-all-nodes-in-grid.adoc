...
Si desea cambiar las direcciones IP y MTU para los nodos en un solo sitio, siga el xref:changing-nodes-network-configuration.adoc[Cambie la configuración de red de los nodos] instrucciones.
...
xref:adding-to-or-changing-subnet-lists-on-grid-network.adoc[Agregar o cambiar listas de subred en Grid Network]

xref:shutting-down-grid-node.adoc[Apague el nodo de grid]
