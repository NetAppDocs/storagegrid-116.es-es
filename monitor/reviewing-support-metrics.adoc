...
IMPORTANT: Las herramientas disponibles en la página Métricas están destinadas al soporte técnico. Algunas funciones y elementos de menú de estas herramientas no son intencionalmente funcionales y están sujetos a cambios. Consulte la lista de xref:commonly-used-prometheus-metrics.adoc[Métricas de Prometheus que se usan habitualmente].
