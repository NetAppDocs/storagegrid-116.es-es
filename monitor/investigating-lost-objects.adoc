...
. Desde un nodo de administración, acceda al registro de auditoría para determinar el identificador único (UUID) del objeto que activó la alerta *objetos perdidos*:
+
...
(Procedimiento para xref:searching-for-and-restoring-potentially-lost-objects.adoc[buscando objetos potencialmente perdidos] Explica cómo usar el ID de nodo para encontrar el nodo de almacenamiento correcto.)
...
Ubicaciones = 0
 a| 
Si no hay ninguna ubicación en la salida, el objeto puede faltar. Puede intentar xref:searching-for-and-restoring-potentially-lost-objects.adoc[busque y restaure el objeto] usted mismo o puede ponerse en contacto con el soporte técnico.

Es posible que el soporte técnico le solicite determinar si hay un procedimiento de recuperación del almacenamiento en curso. Es decir, ¿se ha emitido un comando _repair-data_ en cualquier nodo de almacenamiento y la recuperación sigue en curso? Consulte la información acerca de xref:../maintain/restoring-object-data-to-storage-volume-if-required.adoc[restaurar datos de objeto en un volumen de almacenamiento].
